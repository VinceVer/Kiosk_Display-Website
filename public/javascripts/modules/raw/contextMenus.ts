 ...
 */
const setOnline = async (element: HTMLElement, id: string) => {
    element.querySelector('button[name=online]')!.textContent = "...";
    const response = await sendRequest(`/update/queue.json/${id}`, {value: "ONLINE"}) as any;
 ...
 * @param id - Id of the kiosk, device or application.
 */
const setDelete = async (element: HTMLElement, id: string) => {
    const removeButton = element.querySelector('button[name=remove]')! as HTMLElement;
    ...
    const response = await sendRequest(`/update/queue.json/${id}`, {value: "DELETE"}) as any;
    if (response.message === 'success') {
 ...
 */
function createContextMenu(name: string, template: HTMLTemplateElement, data: Array<{name: string, text?: string, inactive?: boolean, action?: Function}>) {
    const contextMenu = getElementFromTemplate(template);
    if (!usingMobile) contextMenu.dataset.linked_to = name;
    contextMenu.querySelector('.title')!.innerHTML = name;

    for (let buttonData of data) {
        const button = contextMenu.querySelector(`button[name=${buttonData.name}]`) as HTMLButtonElement;
        if (!button) continue;

        if (buttonData.text) button.textContent = buttonData.text;
        if (buttonData.inactive) {button.style.filter = "saturate(0.4) brightness(0.4)"; continue;}
        if (buttonData.action) {
            button.addEventListener('click', () => {
                (buttonData.action as (cm: HTMLElement) => void)(contextMenu);
            });
        }
    }
 ...
 */
function moveContextMenu(menu: HTMLElement, x: number, y: number) {
 ...
 */
export function loadKioskDesktop(event: Event, kiosk: HTMLElement) {
    ...
    const buttons = [
        ...
        },
        {
            ...
            action: (element: HTMLElement) => setOnline(element, kiosk.id)
        },
        {
            name: "remove",
            action: (element: HTMLElement) => setDelete(element, kiosk.id)
        }
    ];
    const contextMenu = createContextMenu(kiosk.id, template as HTMLTemplateElement, buttons);
 ...
 * @param event - Event of the contextmenu, used to override the default event.
 */
export function loadKioskMobile(event: Event, row: HTMLElement) {
    event.preventDefault();
    ...
    const buttons = [
        {
            ...
            action: (element: HTMLElement) => setOnline(element, row.id)
        },
        {
            name: "cancel",
            text: "Cancel",
    ...
    const title = document.getElementById(row.id)?.querySelector('.name')?.innerHTML || "An unexpected error occurred";
    const contextMenu = createContextMenu(title, template as HTMLTemplateElement, buttons);
    ...
    contextMenu.addEventListener('click', (event) => {if (event.target === contextMenu) close(contextMenu)});
}
 ...
 * @param event - Event of the contextmenu, used to override the default event.
 */
export function loadSegmentDesktop(event: Event, segment: HTMLElement) {
    event.preventDefault();
    ...
    for (let statusType in config_data.status_messages) {
        ...
        for (let statusCode in statusObject) {
            if (parseFloat(statusCode) !== parseFloat(segmentObject[statusType])) continue;
            if (!statusObject.codes[statusCode].messages.includes(segmentObject.status_message)) continue;
    ...
    const buttons = [
        {
            ...
            action: (element: HTMLElement) => setOnline(element, segment.id)
        },
        {
            name: "status"
        },
        {
            name: "remove",
            action: (element: HTMLElement) => setDelete(element, segment.id)
        }
    ];
    const contextMenu = createContextMenu(segment.id, template as HTMLTemplateElement, buttons);
    moveContextMenu(contextMenu, mouseEvent.clientX, mouseEvent.clientY);
 ...
 * @param event - Event of the contextmenu, used to override the default event.
 */
export function loadSegmentMobile(event: Event, segment: HTMLElement) {
    event.preventDefault();
    ...
    const title = document.getElementById(segment.id)?.querySelector('.header h3')?.innerHTML || "An unexpected error occurred";
    const contextMenu = createContextMenu(title, template as HTMLTemplateElement, buttons);
...
function close(element: HTMLElement) {
    usingMobile
        ? element.querySelector('div')?.style.removeProperty('height')
 ...
 */
export function setupSwipeActions(element: HTMLElement) {
    let startX = 0,
        startY = 0,
        currentX = 0,
        currentY = 0;
    ...
    const touchMove = (event: TouchEvent) => {
        ...
        if (Math.abs(deltaY) > absDeltaX && !element.classList.contains('selected')) {
            isMoving = false;
            return;
        }

        // Return if user has not swiped far enough.
        if (!isMoving || (absDeltaX < 10 && !element.classList.contains('selected'))) return;

        element.classList.add('selected');
        element.style.translate = deltaX + "px";

        if (absDeltaX > window.innerWidth * 0.4) {
...
document.addEventListener('click', (event) => {
    ...
    (document.querySelectorAll('.contextMenu') as NodeListOf<HTMLElement>).forEach(close);
});