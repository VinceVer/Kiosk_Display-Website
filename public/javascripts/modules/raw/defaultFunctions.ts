 ...
 */
export const createElementWithText = (type: string, text: string, html?: boolean|null) => {
 ...
 */
export const getObjectFromEntries = (iterable: Iterable<any>) => {
 ...
 */
export const getCookie = (id: string) : string|null => {
...
export function setCookie(id: string, value: string|number|null, length?: string|null) {
    const expirationDate = new Date(new Date().getTime() + 365 * 24 * 60 * 60 * 1000);
    if (value === null) length = unix0;
 ...
 */
export const getElementFromTemplate = (template: HTMLTemplateElement) => {
 ...
 */
export const getFormattedPropertyValue = (property: string, value: string|number) : string => {
 ...
 */
export const getRandomValue = (lowerLimit: number, upperLimit: number, lambda?: number, round?: boolean) => {
    ...
    if (round) return Math.floor(lowestValue);
    return lowestValue;
}



/** Returns an RGB value created from a HEX value.
 * @param hex - HEX value to convert to RGB.
 * @param array - If true, returns an array instead of an object.
 */
export function getRgbFromHex(hex: string): {r: number; g: number; b: number};
export function getRgbFromHex(hex: string, array: true): [number, number, number];

export function getRgbFromHex(hex: string, array?: any): {r: number; g: number; b: number} | [number, number, number] {
    hex = hex.replace("#", "");
...
const rgbComponentToHex = (value: string|number) => {
 ...
 */
export function getMobileUrgencyLevel(urgency: any) {
    ...
    if (!yellowBeforePurple) return translatedUrgency;
    switch (translatedUrgency) {
        case "1": translatedUrgency = "2"; break;
        case "2": translatedUrgency = "1"; break
    }
 ...
 */
export const getReadableTime = (timestamp: number|string, monthLength?: number, split?: true) => {
    ...
    if (split) return [`${day} ${month}`,`${hour}:${minute}`];
 ...
 */
export const getReadableTimeSince = (millisecondsDiff: number) => {
 ...
 */
export const getUnixTimestamp = (type?: string) => {
    const currentDate = new Date();

    if (!type) return Math.floor(currentDate.getTime() / 1000);
 ...
 */
export const isArrayOrObject = (variable: any) => {
...
export function loadWallpaper() {
    ...
    if (background_file && !background_file.includes(".")) background_file = `${background_file}.${getCookie(`background_extension[${config_data.location}]`)}`;
 ...
 */
export function makeTextFitContainer(textElement: HTMLElement|Array<HTMLElement>) {
    if (textElement instanceof HTMLElement) textElement = [textElement];
 ...
 */
export const mapNumberToHex = (color1: string, color2: string, value: number) => {
    while (value > 1) value /= 10;

    const Rgb1 = getRgbFromHex(color1, true),
        Rgb2 = getRgbFromHex(color2, true);
 ...
 * @returns a promise.
 */
export const sendRequest = (url: string, body: object) => {
 ...
 */
export function sortElements(container: HTMLElement, selector: string|null, property: string, descending?: boolean|undefined) {
    if (selector === null) {selector = "*"}

    const elements = Array.from(container.querySelectorAll(selector));
    
    const quickSort = (array: Array<any>): any => {
        if (array.length <= 1) {
            return array;
        }
    ...
    if (descending) {sortedElements = sortedElements.reverse()}
 ...
 */
document.querySelector('*')?.addEventListener('contextmenu', (event) => {
    if ((event as KeyboardEvent).ctrlKey) return;
