 ...
 */
export function createGroup(name: string, parent: HTMLDivElement) : HTMLDivElement {
 ...
 */
export function createLocation(name: string, parent: HTMLDivElement) : HTMLDivElement {
 ...
 */
export function createKiosk(name: string, urgenyLevel: number, parent: HTMLDivElement) : HTMLDivElement {
...
export function toggleIgnoreKiosk(id: string) {
 ...
 * 
 */
export function createRow(name: string, icon: string, note: string, parent: HTMLElement) : HTMLElement {
    ...
    row.querySelector('.name div')?.appendChild(createElementWithText('span', name));
 ...
 */
export function alignLocationTitle(groupElement : HTMLDivElement) {
    let maxSpan = 0;
    ...
    groupElement.querySelectorAll('.location').forEach(locationElement => {
        ...
        const locationHtmlElement = locationElement as HTMLElement;
 ...
 */
export function updateRow(kiosk: any) {
    ...
    if (!loadIcons) return;
    const iconElement = row.querySelector('.name .ico') as HTMLElement;
 ...
 * @param newUrgency - New urgency value (null for no response).
 */
export function updateTile(kioskId: string, newUrgency: number|null) {
    const kiosk = document.getElementById(kioskId);
    if (!kiosk) return;

    kiosk.classList.forEach(className => {
        if (className.includes("urgency_")) kiosk.classList.remove(className);
    });
...
window.addEventListener('resize', () => {
    (document.querySelectorAll('.group') as NodeListOf<HTMLDivElement>).forEach(group => {
 ...
 */
(document.querySelectorAll('.dropdown') as NodeListOf<HTMLElement>).forEach(element => {
    ...
    (element.querySelectorAll('.content div') as NodeListOf<HTMLElement>).forEach(item => {
        item.onclick = () => (item.querySelector('a') as HTMLElement).click();
    })
});