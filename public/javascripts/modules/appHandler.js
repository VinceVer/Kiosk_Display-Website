...
const pageHasHeader = document.querySelector('header') ? true : false, pageHasFooter = document.querySelector('footer') ? true : false;
/** Returns the width of the app as if it were minified.
 * @param element - App to convert.
 */
export const getElementBarWidth = (element) => {
 ...
 */
export const getHookToSnapTo = (x, y) => {
 ...
 */
export function minifyApp(app, hook) {
    const minifiedApp = app.cloneNode(true);
    ...
    hook.style.alignItems = minifiedApp.querySelector('table')
        ...
        : "center";
    minifiedApp.querySelectorAll('.flexBar').forEach(element => {
        element.style.removeProperty("margin");
        element.style.display = "inline-flex";
    });
    minifiedApp.querySelectorAll('table, p').forEach(element => {
 ...
 */
export function toggleApp(id, button) {
 ...
 */
export const resetAppCookies = (app) => {
    setTimeout(function () {
...
export function updateApp_5() {
    ...
    for (let row of problemList) {
        ...
        if (hook) {
            hook.innerHTML += newRowHTML;
        }
        const divQuery = 'tr:last-of-type td.name div';
        const iconData = iconStatusDataMap[row.icon];
        const icon = addIcon(problemTable.querySelector(divQuery), iconData.name, iconData.color).cloneNode(true);
...
export function updateApp_S() {
    const dataElement = document.querySelector('.flexBar[for=standby]');
    if (!dataElement || (dataElement.dataset.name === misc_data.standby.name && dataElement.dataset.phone === misc_data.standby.phone))
        return;
...
export function updateApp_T() {
    const hook = document.querySelector('.hook[data-hooked_app=app_T]');
 ...
 */
function closeHook(appId) {
    const hook = document.querySelector(`.hook[data-hooked_app=${appId}]`);
    if (!hook)
        return;
 ...
 */
function placeAppInFront(app) {
    document.querySelectorAll(`.app:not(#${app.id})`).forEach(element => element.style.zIndex = "0");
    app.style.zIndex = "2";
...
export function loadApps() {
    document.querySelectorAll('.hook').forEach(hook => {
        ...
        if (!linkedApp)
            return;
        minifyApp(document.getElementById(linkedApp), hook);
        (_a = document.getElementById(linkedApp + "Button")) === null || _a === void 0 ? void 0 : _a.classList.add('selected');
    });
    document.querySelectorAll('.app').forEach(app => {
        ...
        if (getCookie(`apps.${app.id}.display`) !== null) {
            ...
            button === null || button === void 0 ? void 0 : button.classList.add('selected');
        }
        // Move the app in front of all other apps.
        app.addEventListener('mousedown', () => placeAppInFront(app));
        // Close the app.
        (_a = app.querySelector('.header button')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => toggleApp(app.id, button));
        ...
        document.addEventListener('mousemove', event => {
            ...
            document.querySelectorAll(`.hook:not(#${hook.id})`).forEach(element => {
