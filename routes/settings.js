...
const getSendableData = (req, cookies, customData) => {
...
router.get('/', function(req, res, next) {
  	res.render('settings-home', getSendableData(req, req.cookies));
});

router.get('/grouping/:group?/:location?/:selector?', function(req, res, next) {
	const data = JSON.parse(fs.readFileSync(__dirname+'/../storage/config.json'));

	if (!req.params.group) {return res.render('settings-grouping-home', getSendableData(req, req.cookies));}
	...
	if (data.groups[groupIndex]) {
		...
		res.render('settings-grouping-groups', getSendableData(req, req.cookies, {pageType: pageType, name: name}));
		return;
	} else {
...
router.get('/downloads', function(req, res, next) {
	res.render('settings-downloads', getSendableData(req, req.cookies));
});

router.get('/apps/:appCode?', function(req, res, next) {
	const data = JSON.parse(fs.readFileSync(__dirname+'/../storage/config.json'));

	if (!req.params.appCode) {return res.render('settings-apps-home', getSendableData(req, req.cookies));}

	const appCode = req.params.appCode;

	if (!data.applications[appCode]) {
		res.render('settings-edit', getSendableData(req, req.cookies, {title: "404: App does not (yet) exist. (Looking for ", name: appCode, suffix: ")"}));
	} else {
		res.render('settings-edit', getSendableData(req, req.cookies, {title: "Update the display name of ", name: appCode, suffix: " to:", type: "app"}));
	}
});
...
router.get('/database/conditions/:type?', function(req, res, next) {
	...
	if (req.params.type === "add" && req.query.type) {
		res.render('settings-database-conditions-add', getSendableData(req, req.cookies, {type: req.query.type, title: `Add a ${data.urgency_conditions[req.query.type].text} condition:`}));
	} else if (!req.params.type && req.query.type && req.query.index) {
		if (req.query.type !== "misc") {
			res.render('settings-database-conditions-add', getSendableData(req, req.cookies, {type: req.query.type, title: `Edit ${data.urgency_conditions[req.query.type].text.split(" ")[0]} ${data.urgency_conditions[req.query.type].inputs[req.query.index].name}:`}));
		} else {
			res.render('settings-database-conditions-add', getSendableData(req, req.cookies, {type: req.query.type, title: `Edit ${req.query.index.split("$")[0].split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}:`, input_display: "none"}));
		}
...
router.get('/database/:type', function(req, res, next) {
	...
	res.render('settings-database-'+req.params.type, getSendableData(req, req.cookies, {status_database: statusDatabase}));
});

router.get('/status/:type?/:code?/:message?', function(req, res, next) {
	const data = JSON.parse(fs.readFileSync(__dirname+'/../storage/config.json'));

	if (!req.params.type) {return res.render('settings-status-home', getSendableData(req, req.cookies));}
	...
	if (statusType === "add") {
		res.render('settings-edit', getSendableData(req, req.cookies, {title: "Add a status type:", type: "s_type_add"}));
		next();
	} else {
		let pageType;
		...
		if (data.status_messages[statusType]) {
			...
			res.render('settings-status-types', getSendableData(req, req.cookies, {pageType: pageType}));
			return;
		} else {
